...
<?php
	...
	$sql = "SELECT ID, uname FROM customer WHERE uname=? and passwd_enc=password(?)";
	...
	$login_stat ? $login_stat->bind_result($id, $uname)				: die("sql execute error");

	$login_stat->fetch();

	$_SESSION["username"] = isset($uname) ? $uname : "?";
	$_SESSION["userid"] 	= isset($id) 		? $id		 : -1 ;
	$_SESSION["username"] == '?' 				?
